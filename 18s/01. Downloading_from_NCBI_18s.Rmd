...
```{r}
ncbi_request <- modified_refdb_import_NCBI_pt1("(18s[Gene Name] OR 18S rRNA[Title] OR SSU rRNA[Title]OR 18S ribosomal RNA[Title]) NOT environmental sample[Filter] NOT environmental samples[Filter] NOT environmental[Title] NOT uncultured[Title] NOT unclassified[Title] NOT unidentified[Title] NOT unverified[Title]")
```
...
```{r eval=FALSE}
csv_file <- here::here("18s/ncbi_18s2.csv")
tmp_dir <- here::here("18s/tmp_ncbi_18s2")
...
modified_refdb_import_NCBI_pt2(ncbi_request, csv_output = csv_file, tmp_dir = tmp_dir, start=654000, end=900000-1)
...
```{r eval=FALSE}
csv_file <- here::here("18s/ncbi_18s.csv")
tmp_dir <- here::here("18s/tmp_ncbi_18s")
...
f1 <- modified_refdb_import_NCBI_pt3(here::here("18s/ncbi_18s.csv"))
f2 <- modified_refdb_import_NCBI_pt3(here::here("18s/ncbi_18s2.csv"))
...
saveRDS(combined, file=here::here("18s/18s_ncbi_may24"))
