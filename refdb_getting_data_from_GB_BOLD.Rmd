...
Downloading NCBI COI sequences needs to be done over a few days. Don't expect everything to run smoothly. Expect errors and timeouts.
...
##Jess's code still need to work through##
...
# Load in the data
all_db <- readRDS(here("ref_db/combined_tbl_df.rds"))
nrow(all_db)
```


```{r}
dedup_db <- refdb::refdb_filter_seq_duplicates(all_db)
nrow(dedup_db)
```

```{r}
saveRDS(dedup_db, here("ref_db/dedup_db_tbl_df.rds"))
```

```{r}
dedup_db %>% object.size()
```

```{r}
dedup_db <- readRDS(here("ref_db/dedup_db_tbl_df.rds"))
```
...
```{r}
seqs <- dedup_db[, "sequence",drop=TRUE]
names(seqs) <- 1:nrow(dedup_db)
bioseq::write_fasta(seqs, file = here("ref_db/dedup_ref_db.fasta"),
...
# cd /data/gpfs/projects/punim2065/jess/metabarcoding_ref_db/untrimmed_seq
# head -1001 dedup_ref_db.fasta > subset_test.fasta
# 
# sinteractive --partition=cascade --account=punim2065 --ntasks=1 --cpus-per-task=2 --mem=16384 --time=1:0:0
# # module load cutadapt/4.2
# 
...
# 
# cd /data/gpfs/projects/punim2065/jess/metabarcoding_ref_db/
# 
# cutadapt \
#     --cores 2 \
...
#     -M 450 \
#     --max-n 0.05 \
#     --too-short-output trimming/short_amplicon_too_short.fasta \
#     --too-long-output trimming/short_amplicon_too_long.fasta \
#     --untrimmed-output trimming/short_amplicon_untrimmed.fasta \
#     -o trimming/short_amplicon_trimmed.fasta \
#     untrimmed_seq/subset_test.fasta \
#     > trimming/short_amplicon_cutadapt.log
```


```
sinteractive --partition=cascade --account=punim2065 --ntasks=1 --cpus-per-task=4 --mem=32768 --time=8:0:0
conda activate metabarcoding
cd /data/gpfs/projects/punim2065/jess/metabarcoding_ref_db
cutadapt \
    --cores 4 \
    ...
    --untrimmed-output trimming/short_amplicon_untrimmed.fasta \
    -o trimming/short_amplicon_trimmed.fasta \
    untrimmed_seq/dedup_ref_db.fasta \
    > trimming/short_amplicon_cutadapt.log
...
cutadapt \
    ...
    -M 550 \
    --max-n 0.05 \
    --too-short-output trimming/${ampliconGroup}_amplicon_too_short.fasta \
    --too-long-output trimming/${ampliconGroup}_amplicon_too_long.fasta \
    --untrimmed-output trimming/${ampliconGroup}_amplicon_untrimmed.fasta \
    -o trimming/${ampliconGroup}_amplicon_trimmed.fasta \
    untrimmed_seq/dedup_ref_db.fasta \
    > trimming/${ampliconGroup}_amplicon_cutadapt.log
...
```{r}
short_fasta <- bioseq::read_fasta(file=here("ref_db/short_amplicon_trimmed.fasta"))
```
...
rc_ids <- names(short_fasta)[! names(short_fasta) %in% rownames(dedup_db)]
length(rc_ids)
head(rc_ids)
```
...
```{r}
# Check
short_fasta["4329 rc"] %>% as.character
short_fasta["4329 rc"] %>% seq_reverse %>% seq_complement %>% as.character
dedup_db["4329", "sequence", drop=TRUE] %>% as.character
```

```{r}
short_db <- dedup_db[str_remove(names(short_fasta), " rc$"),]
...
db_files <- c(
  "short_db_2023-07" = here("ref_db/short_amplicon_trimmed.fasta"),
  "long_db_2023-07" = here("ref_db/long_amplicon_trimmed.fasta"),
  "left_db_2023-07" = here("ref_db/left_amplicon_trimmed.fasta"),
  "right_db_2023-07" = here("ref_db/right_amplicon_trimmed.fasta")
)
```
...
for (i in seq_along(db_files)) {
  ...
  db <- dedup_db[str_remove(names(fasta), " rc$"),]
  db$sequence <- fasta
...
conda activate cutadapt
cd ~/projects/metabarcoding_pipeline/ref_db/
