...
NOTE: This section is not currently working in refdb. These scripts are problematic for the download of data from GenBank (the link gets lost). Jess has modified the code make this step work but it is still problematic. Downloading NCBI COI sequences needs to be done over a few days. Don't expect everything to run smoothly. Expect errors and timeouts.
...
# Step 3: Retriving the last saved version of NCBI GenBank raw data file (if starting from this point)

```{r}
NCBI_data <- readRDS("/mnt/galaxy/home/jess/projects/metabarcoding_pipeline/ref_db/combined_tbl_df.rds")
```

# BOLD data on NCBI is named 'sp. BOLD:' should be converted to 'sp. B-'

```{r}
NCBI_data$species <- gsub("sp. BOLD:", "sp. B-", NCBI_data$species)

print (sum(str_detect(NCBI_data$species, 'sp. BOLD:')) > 0)
```
...
# Step 5: Harmonize taxonomic name nomenclature
...
NOTE: if a download of the data is not require please skip to step 9 import the last version of BOLD data downlaoaded and cleaned.
...
# save merged raw data from BOLD library (note change date for new downloads)
```{r}
...
```{r}
all_bold <- readRDS("~/Documents/DNA_reference_db_construction/refdb/Input_data_Oct23/all_bold_oct23.rds")
```
...
#Step 2: Remove duplicates. It is good to write this file prior to preforming other clean_up steps
...
#Step 3: Remove sequence alignment gaps from BOLD library:
```{r}
...
#Step 4: Remove side N's (removes N's at start or end of sequence)
...
#Step 5: Ensures that the same taxonomic classification are used (in specified columns if needed)
...
#Step 6: Remove extra bits on the end of species names (CONSIDERED REMOVING THIS STEP)
...
#Step 7:Plot sequence length
```{r}
refdb_plot_seqlen_hist(all_bold_clean)
```

#Step 8: Filter for CO1 only. BOLD contains sequences from multiple gene regions
The scripts below can be used to group sequences on BOLD for difference gene regions
```{r}
all_bold_clean_COI <- subset(all_bold_clean, markercode=="COI-5P")
#all_bold_clean_18S <- subset(all_bold_clean, markercode=="18S")
#all_bold_clean_ITS <- subset(all_bold_clean, markercode=="ITS")
#all_bold_clean_12S <- subset(all_bold_clean, markercode=="12S")
#all_bold_clean_ND2 <- subset(all_bold_clean, markercode=="ND2")
#all_bold_clean_16S <- subset(all_bold_clean, markercode=="16S")
```

# plot
```{r}
refdb_plot_seqlen_hist(all_bold_clean_COI)
```
...
```{r}
refdb_plot_tax_treemap(all_bold_clean_COI, cols = 1)
```

Alternatively you can represent this information with a taxonomic tree. These functions have several parameters to control what is represented in the plot (taxonomic ranks, colors, etc.).

```{r}
refdb_plot_tax_tree(all_bold_clean_COI)
```
...
```{r}
write_rds(all_bold_clean_COI,"~/Documents/DNA_reference_db_construction/refdb/Output_data_Oct23/all_bold_clean_Oct23.rds")
```

# import cleaned BOLD data (if required)
```{r}
all_bold_clean_COI <- readRDS("~/Documents/DNA_reference_db_construction/refdb/Output_data_Oct23/all_bold_clean_Oct23.rds")
```
...
priv_lib[1141,6] <- "Larsia sp. B-AEN3053"
...
priv_lib[1361,6] <- "Cryptochironomus sp. B-ACS5094"
...
```{r}
write_rds(priv_lib_dup, "~/Documents/DNA_reference_db_construction/refdb/Output_data_Oct23/priv_lib_Oct23_dup.rds")
```

# importing (if required)

```{r}
priv_lib_dup <- readRDS("~/Documents/DNA_reference_db_construction/refdb/Output_data_Oct23/priv_lib_Oct23_dup.rds")
```

# set fields for priv_lib_dup
```{r}
priv_lib_dup <- refdb_set_fields(priv_lib_dup,
                             taxonomy = c(
                               phylum = "phylum",
                               class = "class",
                               order = "order",
...
refdb_get_fields(priv_lib_dup)
```

```{r}
refdb_export_mothur(priv_lib_dup, "~/Documents/DNA_reference_db_construction/refdb/Output_data_Oct23/priv_lib_dup.txt")
refdb_export_mothur(priv_lib_dup, "~/Documents/DNA_reference_db_construction/refdb/Output_data_Oct23/priv_lib_dup.fasta")
```
...
```{r}
all_bold_priv_NCBI <- refdb_merge(priv_lib_dup, NCBI_data_clean, all_bold_clean_COI, keep = "fields_shared")
```

```{r}
rm(all_bold_clean_COI)
rm(NCBI_data_clean)
rm(priv_lib_dup)
```
...
#import in library final clean library (if required)    
...
## issues from refdb in the final library
...
write_rds(all_bold_priv_NCBI_final,"~/Documents/DNA_reference_db_construction/refdb/Output_data_Oct23/all_bold_priv_NCBI_final_Oct23.rds")
#not working
refdb_export_mothur(all_bold_priv_NCBI_final, "~/Documents/DNA_reference_db_construction/refdb/final_library/all_bold_priv_NCBI_final.fasta")
```
...
# Load in the data
all_db <- readRDS("~/Documents/DNA_reference_db_construction/refdb/Output_data_Oct23/all_bold_priv_NCBI_final_Oct23.rds")
nrow(all_db)
```
...
export1 <- all_db[1:2859080, 1:10]
export2 <- all_db[2859081:5718160, 1:10]
```
...
cutadapt \
    ...
    -M 450 \
    --max-n 0.05 \
    --too-short-output ~/Documents/DNA_reference_db_construction/refdb/final_library/trim_seq/trimming/${ampliconGroup}_amplicon_too_short.fasta \
    --too-long-output ~/Documents/DNA_reference_db_construction/refdb/final_library/trim_seq/trimming/${ampliconGroup}_amplicon_too_long.fasta \
    --untrimmed-output ~/Documents/DNA_reference_db_construction/refdb/final_library/trim_seq/trimming/${ampliconGroup}_amplicon_untrimmed.fasta \
    -o ~/Documents/DNA_reference_db_construction/refdb/final_library/trim_seq/trimming/${ampliconGroup}_amplicon_trimmed.fasta \
    ~/Documents/DNA_reference_db_construction/refdb/final_library/trim_seq/NBP1.fasta \
    > ~/Documents/DNA_reference_db_construction/refdb/final_library/trim_seq/trimming/${ampliconGroup}_amplicon_cutadapt.log
...
cd ~/Documents/DNA_reference_db_construction/refdb/final_library/trim_seq
cd ~/Documents/DNA_reference_db_construction/refdb/final_library/trim_seq/trimming
cutadapt \
    --cores 4 \
    -g ACWGGWTGRACWGTNTAYCC...GCHGGDGCHATYACHATRYT \
    -e 0.2 \
...
```{r}
short_fasta <- bioseq::read_fasta(file="~/Documents/DNA_reference_db_construction/refdb/final_library/trim_seq/trimming/short_amplicon_trimmed.fasta")
```
...
```{r}
# Check
short_fasta["620 rc"] %>% as.character
short_fasta["620 rc"] %>% seq_reverse %>% seq_complement %>% as.character
all_db["620", "sequence", drop=TRUE] %>% as.character
```
