...
If a downloading from the NCBI GenBank data is not require please skip to 'Step 7' import the last version of the cleaned NCBI GenBank data using the import script.
...
You may have notice that the search interface is a bit different. This is because here we rely on another package (bold, Chamberlain 2020) to download the data. You can check the manual of refdb::refdb_import_BOLD to see the different arguments available.
Now let’s download data from the BOLD database. Data needs downloaded for animals groups at different taxonomic levels depending on the number of sequences in that group. For small groups, it can be downloaded at phlyum level for larger groups with more sequence records, we'll need to follow the BOLD "large data" instructions which limits the size of data download requests (see ?refdb::refdb_import_BOLD). I have configured this below useing trial and error to enable data from all animal groups to be download.
...
#Step 2: Load control spike sample
```{r}
...
#Step 3: Merging all BOLD into one dataframe
```{r}
...
# add superkingdom column to bring in line with NCBI GenBank data
bold_plafun_data <- Proteobacteria_bold %>%
...
```{r}
write_rds(bold_data,"~/Documents/DNA_reference_db_construction/refdb/Input_data_Oct23/bold_data_oct23.rds")
```

# reload all BOLD document for cleaning (if needed)
```{r}
bold_data <- readRDS("~/Documents/DNA_reference_db_construction/refdb/Input_data_Oct23/bold_data_oct23.rds")
```

# Step 5: Setting fields for all BOLD data
```{r}
...
Note: DO NOT use remove refdb_clean_tax_NA. There is a bug as it removes interm names and the letters 'na'  within an taxonomic names. Another script is provided to remove 'NA' from the library so that 'NA's don't appear in taxonomic information of the final library files.

#Step 4: Remove duplicates. It is good to write this file prior to preforming other clean_up steps
This step removes a large amount of data it looks for records with identical sequences and taxonomic information and removes all duplications. Expect the number of records to reduce by 30-50%.
```{r}
...
```{r}
priv_lib <- read_csv ("~/Documents/DNA_reference_db_construction/refdb/Input_data_Oct23/priv_lib_Nov23.csv")
```
...
```{r}
saveRDS(priv_lib_clean, "~/Documents/DNA_reference_db_construction/refdb/Output_data_Oct23/priv_lib_Nov23_cleaned.rds")
```

# importing (if required)

```{r}
priv_lib_clean <- readRDS("~/Documents/DNA_reference_db_construction/refdb/Output_data_Oct23/priv_lib_Oct23_cleaned.rds")
```
...
```{r}
# fix outdated taxonomic names (Changing Ancylidae to Planorbidae, Sphaeriidae to Pisidiidae, Corbiculidae to Cyrenidae, Brentidae to Nanophyidae) that remain in the database
...
colnames(all_bold_priv_ncbi)[colnames(all_bold_priv_ncbi) == "all_bold_priv_ncbi"] ="sequence"
```  

# export final library as a dataframe
```{r}
saveRDS(all_bold_priv_ncbi,"~/Documents/DNA_reference_db_construction/refdb/final_library/all_bold_priv_NCBI_final_clean_Oct23.rds")

all_bold_priv_ncbi <- readRDS("~/Documents/DNA_reference_db_construction/refdb/final_library/all_bold_priv_NCBI_final_clean_Oct23.rds")
```
...
# add a an underscore to species to replace space in species names
...
# save fasta 
refdb_export_dada2(export, "~/Documents/DNA_reference_db_construction/refdb/final_library/final_lib_export_1.fasta", mode = "taxonomy")
...
# save fasta 
refdb_export_dada2(export2, "~/Documents/DNA_reference_db_construction/refdb/final_library/final_lib_export_2.fasta", mode = "taxonomy")
...
# save fasta 
refdb_export_dada2(export3, "~/Documents/DNA_reference_db_construction/refdb/final_library/final_lib_export_3.fasta", mode = "taxonomy")
...
#fasta1 <- bioseq::read_fasta("~/Documents/DNA_reference_db_construction/refdb/final_library/final_lib_export_1.fasta")

#fasta2 <- bioseq::read_fasta("~/Documents/DNA_reference_db_construction/refdb/final_library/final_lib_export_2.fasta")

#fasta3 <- bioseq::read_fasta("~/Documents/DNA_reference_db_construction/refdb/final_library/final_lib_export_3.fasta")
...
# Load in the data
all_db <- readRDS("~/Documents/DNA_reference_db_construction/refdb/final_library/all_bold_priv_NCBI_final_clean_nov23.rds")
...
##Needs more work (this is a library that would be used for DNA barcoding only) 
#LCO amplicon
ampliconGroup=LCO
cutadapt \
    --cores 4 \
...
```{r}
# read in trimmed short fasta
right_fasta <- bioseq::read_fasta(file="~/Documents/DNA_reference_db_construction/refdb/final_library/trimming/right_amplicon_trimmed.fasta")
```
...
```{r}
# Some sequences are now the reverse complement these need to be corrected
rc_ids <- names(right_fasta)[! names(right_fasta) %in% rownames(all_db)]
length(rc_ids)
head(rc_ids)
```
...
```{r}
# Check sequences in reverse compliment (rc)
right_fasta["110 rc"] %>% as.character
right_fasta["110 rc"] %>% seq_reverse %>% seq_complement %>% as.character
all_db["110", "sequence", drop=TRUE] %>% as.character
```

```{r}
# remove 'rc' from sequences
right_db <- all_db[str_remove(names(right_fasta), " rc$"),]
right_db$sequence <- right_fasta
```

```{r}
#table(is.na(right_db$id))
```
# long amplicon
```{r}
# read in trimmed short fasta
long_fasta <- bioseq::read_fasta(file="~/Documents/DNA_reference_db_construction/refdb/final_library/trimming/long_amplicon_trimmed.fasta")
```
...
# some sequences are now the reverse complement (rc) these need to be corrected
```{r}
# find rc sequences
rc_ids <- names(long_fasta)[! names(long_fasta) %in% rownames(all_db)]
length(rc_ids)
head(rc_ids)
```

```{r}
# reverse complement rc sequences - make sure number corresponds to the first record number i.e., 110 rc
long_fasta["110 rc"] %>% as.character
long_fasta["110 rc"] %>% seq_reverse %>% seq_complement %>% as.character
all_db["110", "sequence", drop=TRUE] %>% as.character
```

```{r}
# remove 'rc' from sequences
long_db <- all_db[str_remove(names(long_fasta), " rc$"),]
long_db$sequence <- long_fasta
```
...
```{r}
# read in trimmed short fasta
left_fasta <- bioseq::read_fasta(file="~/Documents/DNA_reference_db_construction/refdb/final_library/trimming/left_amplicon_trimmed.fasta")
```
...
```{r}
# find rc sequences
rc_ids <- names(left_fasta)[! names(left_fasta) %in% rownames(all_db)]
length(rc_ids)
head(rc_ids)
```
...
```{r}
# reverse complement rc sequences - make sure number corresponds to the first record number i.e., 110 rc
left_fasta["110 rc"] %>% as.character
left_fasta["110 rc"] %>% seq_reverse %>% seq_complement %>% as.character
all_db["110", "sequence", drop=TRUE] %>% as.character
```

```{r}
# remove 'rc' from sequences
left_db <- all_db[str_remove(names(left_fasta), " rc$"),]
left_db$sequence <- left_fasta
```
...
db_files <- c(
  "short_db_2023-07" = "~/Documents/DNA_reference_db_construction/refdb/final_library/trimming/short_amplicon_trimmed.fasta",
  "long_db_2023-07" = "~/Documents/DNA_reference_db_construction/refdb/final_library/trimming/long_amplicon_trimmed.fasta",
  "left_db_2023-07" = "~/Documents/DNA_reference_db_construction/refdb/final_library/trimming/left_amplicon_trimmed.fasta",
  "right_db_2023-07" = "~/Documents/DNA_reference_db_construction/refdb/final_library/trimming/right_amplicon_trimmed.fasta")
```
