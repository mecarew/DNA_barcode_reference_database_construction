...
# Change BOLD naming in GenBank data
BOLD data on NCBI is named 'sp. BOLD:' should be converted to 'sp. B-' to be consistent with the naming in the MW bug database and our private DNA barcode library (from our own DNA barcoding)
...
```{r}
rm(all_bold_priv_NCBI_fima)
```
# exporting to other formats (files are too large)

```{r}
#not working
refdb_export_mothur(all_bold_priv_NCBI_final, "~/Documents/DNA_reference_db_construction/refdb/final_library/all_bold_priv_NCBI_final")
#not working
refdb_export_dada2(all_bold_priv_NCBI_final, "~/Documents/DNA_reference_db_construction/refdb/final_library/all_bold_priv_NCBI_final.fasta", mode = "taxonomy")
```
...
bioseq::write_fasta(seqs[parts == 1], file = "~/Documents/DNA_reference_db_construction/refdb/final_library/all_NBP.fasta",
                    line_length = 80, block_length=Inf, append=FALSE)
bioseq::write_fasta(seqs[parts == 2], file = "~/Documents/DNA_reference_db_construction/refdb/final_library/all_NBP.fasta",
                    line_length = 80, block_length=Inf, append=TRUE)
bioseq::write_fasta(seqs[parts == 3], file = "~/Documents/DNA_reference_db_construction/refdb/final_library/all_NBP.fasta",
                    line_length = 80, block_length=Inf, append=TRUE)
bioseq::write_fasta(seqs[parts == 4], file = "~/Documents/DNA_reference_db_construction/refdb/final_library/all_NBP.fasta",
                    line_length = 80, block_length=Inf, append=TRUE)
```
...
cutadapt \
    ...
    ~/Documents/DNA_reference_db_construction/refdb/final_library/all_NBP.fasta \
...
cutadapt \
    --cores 4 \
    -g GGTCAACAAATCATAAAGAYATYGG...GGRTANACWGTYCAWCCWG \
    -e 0.2 \
    --revcomp \
    -m 200 \
    -M 550 \
...
rc_ids <- names(short_fasta)[! names(short_fasta) %in% rownames(all_NBP)]
length(rc_ids)
head(rc_ids)
```
...
all_NBP["620", "sequence", drop=TRUE] %>% as.character
```

```{r}
short_db <- all_NBP[str_remove(names(short_fasta), " rc$"),]
