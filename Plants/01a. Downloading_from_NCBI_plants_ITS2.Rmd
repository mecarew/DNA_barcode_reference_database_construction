...
----------------------------------------------------------------------------
# SECTION 1: Getting data from NCBI GenBank for ITS2
...
```{r}
ncbi_request <- modified_refdb_import_NCBI_pt1("(ITS2[Gene Name] OR internal transcribed spacer 2[Title] OR ITS2[Title]) NOT environmental sample[Filter] NOT environmental samples[Filter] NOT environmental[Title] NOT uncultured[Title] NOT unclassified[Title] NOT unidentified[Title] NOT unverified[Title]")
```
...
```{r eval=FALSE}
csv_file <- here::here("ncbi_its2.csv")
tmp_dir <- here::here("ncbi_its2")
...
modified_refdb_import_NCBI_pt2(ncbi_request, csv_output = csv_file, tmp_dir = tmp_dir, start=591400, end=600000-1)
...
f1 <- modified_refdb_import_NCBI_pt3(here("ncbi_ITS2_part1.csv"))
f2 <- modified_refdb_import_NCBI_pt3(here("ncbi_its2.csv"))
...
```{r}
# Combine everything
combined_with_dups <- rbind(f1, f2)
...
#data <- read.csv("~/git/DNA_barcode_reference_database_construction/Plants/ncbi_its2.csv")

# I think we should name the file with the database name and date (i.e., all NCBI_Oct23)
saveRDS(combined, file= "~/git/DNA_barcode_reference_database_construction/Plants/combined_ncbi_its2_may24.rds")
...
```{r}
ncbi_data <- readRDS("~/git/DNA_barcode_reference_database_construction/Plants/combined_ncbi_ITS2.rds")
```
