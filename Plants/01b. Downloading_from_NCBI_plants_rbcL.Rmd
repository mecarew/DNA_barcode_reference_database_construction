...
# Step 2: Download sequence data for rbcL 
```{r}
ncbi_request <- modified_refdb_import_NCBI_pt1("(((rbcL) OR ribulose-1,5-bisphosphate carboxylase/oxygenase) OR ribulose-1,5-bisphosphate carboxylase oxygenase) AND (chloroplast[Filter] OR plastid[Filter]) NOT environmental sample[Filter] NOT environmental samples[Filter] NOT environmental[Title] NOT uncultured[Title] NOT unclassified[Title] NOT unidentified[Title] NOT unverified[Title]")
```
...
modified_refdb_import_NCBI_pt2(ncbi_request, csv_output = csv_file, tmp_dir = tmp_dir, start=200000, end=300000-1)

modified_refdb_import_NCBI_pt2(ncbi_request, csv_output = csv_file, tmp_dir = tmp_dir, start=300000, end=400000-1)
...
saveRDS(f2, file=here::here("Plants/combined_ncbi_rbcL.rds"))
...
```{r}
# save cleaned data
saveRDS(ncbi_data, file=here::here("Plants/rbcL_ncbi/ncbi_data_rbcL_may_24.rds"))
