...
```{r}
all_db <- readRDS(here::here("Plants/combined_ncbi_trnl_may24.rds"))
```

```{r}
# find and remove duplicates
duplicated_ids <- duplicated(all_db$id)

all_db <- all_db[!duplicated_ids, ]
```
...
bioseq::write_fasta(seqs[parts == 1], file = here::here("Plants/trimming/trnl.fasta"),
                    line_length = 80, block_length=Inf, append=FALSE)
bioseq::write_fasta(seqs[parts == 2], file = here::here("Plants/trimming/trnl.fasta"),
                    line_length = 80, block_length=Inf, append=TRUE)
bioseq::write_fasta(seqs[parts == 3], file = here::here("Plants/trimming/trnl.fasta"),
                    line_length = 80, block_length=Inf, append=TRUE)
bioseq::write_fasta(seqs[parts == 4], file = here::here("Plants/trimming/trnl.fasta"),
                    line_length = 80, block_length=Inf, append=TRUE)
```
...
command <- paste(
    "cutadapt -g GGGCAATCCTGAGCCAA...GATAGGTGCAGAGACTCAATGG -e 0.10 --revcomp -m 20 -M 200 --max-n 0.05 ",
...
# Construct and execute the Cutadapt command
command <- paste(
    "cutadapt -g GGGCAATCCTGAGCCAA...GATAGGTGCAGAGACTCAATGG -e 0.50 --revcomp -m 20 -M 200 --max-n 0.05 ",
...
# Construct and execute the Cutadapt command
command <- paste(
    "cutadapt -g GGGCAATCCTGAGCCAA...GATAGGTGCAGAGACTCAATGG -e 0.48 --revcomp -m 20 -M 450 --max-n 0.05 ",
...
trimmed_files <- c(
    paste0(output_dir, "/trimming/trnl_amplicon_trimmed1.fasta"),
    paste0(output_dir, "/trimming/trnl_amplicon_trimmed2.fasta"),
    paste0(output_dir, "/trimming/trnl_amplicon_trimmed3.fasta")
)

# Output file to create
output_file <- paste0(output_dir, "/trnl_amplicon_trimmed.fasta")
...
#db_files <- c(
#  "trnl_db_2024-05" = "~/git/DNA_barcode_reference_database_construction/Plants/trnl_amplicon_trimme#d.fasta")

# Output prefix name and input file path
db_files <- c(
  "trnl_db_2024-05" = "~/git/DNA_barcode_reference_database_construction/Plants/trimming/trnl.fasta")
```
