---
title: "Creating customised DNA reference databases - downloading from NCBI ITS2"
...
----------------------------------------------------------------------------
# SECTION 1: Getting data from NCBI GenBank for trnL

The refdb packages uses the rentrez package (Winter 2017) to interface with NCBI servers. F
...
# Step 2: Download sequence data for ITS
```{r}
ncbi_request <- modified_refdb_import_NCBI_pt1("(trnL OR tRNA-Leu OR trn-L OR trn L) AND (chloroplast[Filter] OR plastid[Filter]) NOT environmental sample[Filter] NOT environmental samples[Filter] NOT environmental[Title] NOT uncultured[Title] NOT unclassified[Title] NOT unidentified[Title] NOT unverified[Title]")
```
...
```{r eval=FALSE}
csv_file <- here::here("ncbi_trnl.csv")
tmp_dir <- here::here("ncbi_trnl")

# e.g. Run commands in batches like so:
#modified_refdb_import_NCBI_pt2(ncbi_request, csv_output = csv_file, tmp_dir = tmp_dir, start=0, end=100000)

#modified_refdb_import_NCBI_pt2(ncbi_request, csv_output = csv_file, tmp_dir = tmp_dir, start=100000, end=300000-1)

modified_refdb_import_NCBI_pt2(ncbi_request, csv_output = csv_file, tmp_dir = tmp_dir, start=268400, end=400000-1)

modified_refdb_import_NCBI_pt2(ncbi_request, csv_output = csv_file, tmp_dir = tmp_dir, start=900000, end=1200000-1)

modified_refdb_import_NCBI_pt2(ncbi_request, csv_output = csv_file, tmp_dir = tmp_dir, start=1200000, end=1400000-1)
...
#modified_refdb_import_NCBI_pt2(ncbi_request, csv_output = csv_file, tmp_dir = tmp_dir, start=400000, end=500000-1)
...
# Remove duplicates
dups <- rev(duplicated(rev(ncbi_trnl$id)))
combined <- ncbi_trnl[! dups,]
table(duplicated(combined$id))

#data <- read.csv("~/git/DNA_barcode_reference_database_construction/Plants/ncbi_trnl.csv")

# I think we should name the file with the database name and date (i.e., all NCBI_Oct23)
saveRDS(ncbi_trnl, file= "~/git/DNA_barcode_reference_database_construction/Plants/combined_ncbi_trnl_may24.rds")
...
# Step 3: Retriving the last saved version of NCBI GenBank raw trnL data file (if starting from this point)
```{r}
ncbi_data <- readRDS("~/git/DNA_barcode_reference_database_construction/Plants/combined_ncbi_trnl_may24.rds")
```
...
ncbi_data <- refdb::refdb_set_fields(ncbi_data,
...
refdb::refdb_get_fields(ncbi_data)
```
...
```{r}
ncbi_data <- refdb::refdb_filter_seq_duplicates(ncbi_data)
```
...
```{r}
refdb::refdb_plot_seqlen_hist(ncbi_data)
```
...
```{r}
refdb::refdb_plot_tax_treemap(ncbi_data, cols = 2)
```
...
```{r}
refdb::refdb_report(ncbi_data, view=TRUE)
```                       
